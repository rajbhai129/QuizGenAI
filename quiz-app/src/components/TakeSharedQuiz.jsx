...
const TakeSharedQuiz = () => {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/api/quiz/shared?quizId=${quizId}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/quiz/shared/submit?quizId=${quizId}`, {
        method: 'POST',
        headers: {
