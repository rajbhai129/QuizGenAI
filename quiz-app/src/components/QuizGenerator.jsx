    ...
    <section className="bg-gradient-to-br from-purple-100 via-pink-100 to-blue-100 py-20 min-h-[100vh] flex items-center justify-center px-4">
            ...
            {quizData && (
              <div className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <div className="text-2xl font-bold text-blue-600">{quizData.length}</div>
                    <div className="text-xs text-gray-600">Questions</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-green-600">
                      {quizData.filter(q => q.type === 'single').length}
                    </div>
                    <div className="text-xs text-gray-600">Single Choice</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-purple-600">
                      {quizData.filter(q => q.type === 'multiple').length}
                    </div>
      ...
      <div className="w-full max-w-3xl bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl p-10 relative overflow-hidden border border-white">
        ...
        >
          ...
          {/* Quick Take Quiz Button (when quiz data is available) */}
          {quizData && (
            <button
              type="button"
              onClick={() => navigate('/take')}
              className="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white font-bold py-3 rounded-xl shadow-lg hover:scale-105 hover:shadow-2xl transition-all duration-200 flex justify-center items-center text-lg mt-4"
            >
              🎯 Take This Quiz Now
            </button>
          )}
